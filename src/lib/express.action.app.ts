...
import { ExpressActionRouter } from "./express.action.router";
import { StartupInfo } from "./startup.info";

export class ExpressActionApp {
    constructor(
        ...
        this.server = this.app.listen(startupInfo.port || 3000, startupInfo.host);
    }
    ...
    public add(path: string, router: ExpressActionRouter) {
        this.root.use(path, router.router);
    }
...
const logger = getLogger(ExpressActionApp.name);
